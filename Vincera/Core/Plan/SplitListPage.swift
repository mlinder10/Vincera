...
struct PremiumSplitCell: View {
    @EnvironmentObject private var pStore: ProductStore
    var split: PremiumSplit
    
    var body: some View {
        SplitCell(split: Binding(get: { split.split }, set: { _ in }))
            .overlay {
                if !pStore.hasPurchased(split.productId) {
                    lockedOverlay
                }
            }
    }
