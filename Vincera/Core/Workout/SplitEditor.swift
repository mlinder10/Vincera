...
struct SplitEditor: View {
    ...
    private func handleSave() {
        do {
            ...
            if store.split.list.contains(where: { $0.id == split.id }) {
                try store.split.edit(split.toWriter())
            } else {
                try store.split.create(split.toWriter())
            }
            ...
            LazyVStack {
                ...
                DraggableForEach($day.wrappers, withDividers: true) { wrapper in
                    ExerciseView(
                        wrapper: wrapper,
                        previous: previous,
                        hiddenIds: day.wrappers.flattened().map({ $0.listId }),
